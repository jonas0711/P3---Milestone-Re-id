...
The triplet loss function works on three samples at a time \cite{tripletlossadintro, tripletlossintro}: 

\begin{itemize}
    \item \texttt{anchor}
    \item \texttt{postive}, a sample with the same label as the \texttt{anchor} 
    \item \texttt{negative}, a sample with a different label than the \texttt{anchor}
\end{itemize}

\noindent The goal is to minimize the distance between the \texttt{anchor} and the \texttt{positive} sample, while expanding the gap between the \texttt{anchor} and the \texttt{negative} sample. This way the model learns to group similar cases together and keep different examples far apart. This is useful for person \acs{ReID}, 
where the goal is to generate embeddings that cluster images of the same person together, while keeping embeddings of different people separated. 
...
\subsection*{mAP}
\ac{mAP} is a usefull metric commonly used to examine the performance of object detection systems, such as a \ac{ReID} model. The \ac{mAP} metric is made up of multiple other sub metrics, such as \ac{IoU}, Recall, and Precision, that are important to understand to grasp how the \ac{mAP} works, and how it is correctly calculated. 



To correctly calculate the \ac{mAP} metric for 
