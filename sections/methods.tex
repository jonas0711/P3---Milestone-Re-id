...
\\\\
As \acs{EDSR} is a deep and wide model, residual scaling is used such that the output of each residual block is multiplied by a small factor before being added to the skip connection, in order to stabilize the training. The upsampling stage is implemented by learnable upsampling modules placed at the end of the network, so that most of the computation is performed in the low-resolution space, effectively meaning only the final layers project the feature maps to high resolution. 
\\\\
...
The triplet loss function works on three objects at a time: 
\begin{itemize}
    \item \texttt{anchor}
