...
Two training strategies were employed: training from scratch and finetuning from pretrained weights.

\subsubsection{Training from Scratch}
The model trained from scratch was initialized with random weights and trained on cropped RELLISUR data. The training configuration used an Adam optimizer with $\beta_1 = 0.9$, $\beta_2 = 0.999$, and $\epsilon = 10^{-8}$. The learning rate was set to $1 \times 10^{-4}$ and kept constant throughout training.

\subsubsection{Finetuning from DIV2K}
The finetuned model started from weights pretrained on the \ac{DIV2K} dataset. For finetuning, the learning rate was reduced by a factor of 10 to $1 \times 10^{-5}$ to prevent catastrophic forgetting of the pretrained features. The same Adam optimizer configuration was used.
...
Table \ref{tab:edsr-hyperparameters} summarizes the complete set of training hyperparameters for both training strategies.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}\hline
        \cellcolor[HTML]{D8E9F7}\textbf{Hyperparameter} & \cellcolor[HTML]{D8E9F7}\textbf{Scratch} & \cellcolor[HTML]{D8E9F7}\textbf{Finetuned} \\\hline
        Training Dataset & RELLISUR Cropped & DIV2K + RELLISUR Cropped \\\hline
