...
This project addresses the problem statement of designing a proof of concept system person \ac{ReID} system improved through \ac{SR} Integration. The results demonstrates ? ***** 
...
%SR forbedringer
%Implementere auxiliary loss i End-to-end training så SR også optimeres på reconstruction quality
...
For practical deployment, several improvements are necessary . The The pipeline should be optimized for real-time inference to enable immediate processing of surveillance footage during active investigations **MAybe not** . Exploring edge deployment options would allow the system to run on local hardware, reducing latency and dependency on network connectivity. A comprehensive logging system should be implemented to track when and by whom the system is accessed, supporting transparency and accountability in accordance with ethical guidelines. Establishing a feedback loop from practical use would enable continuous improvement based on real-world performance and user needs. Finally, implementing end-to-end encryption for both data storage and transmission is essential to protect the sensative personal data processed by the system 
