...
\subsection{EDSR}
The selection of EDSR as the SR architecture was motivated by its established performance in the low-distortion region of the perception-distortion trade-off, as presented in Section \ref{subsec:perception-distortion-tradeoff}. This design choice aligned with the project's objective of preserving discriminative features for downstream ReID rather than prioritizing perceptual quality that could introduce hallucinated details.
The independent evaluation of EDSR demonstrated that the model successfully learned to upscale images beyond simple interpolation methods. The scratch model achieved a PSNR of 28.25 dB compared to 27.15 dB for bicubic interpolation, representing an improvement of 1.10 dB. While this improvement confirms that EDSR provides meaningful reconstruction quality gains, the magnitude of improvement is relatively modest compared to what larger EDSR configurations or more recent SR architectures might achieve. The baseline configuration with 64 features and 16 residual blocks was chosen to balance computational efficiency with reconstruction quality, but this conservative choice may have limited the model's capacity to learn more complex degradation patterns.
An unexpected outcome was observed in the two-stage training strategy. The hypothesis that finetuning on DIV2K would improve generalization was not supported by the results, as the scratch model trained solely on RELLISUR outperformed the finetuned model by 0.16 dB on the RELLISUR test set. This suggests that the domain gap between real-world degradations in RELLISUR and synthetically generated degradations in DIV2K was significant enough that finetuning introduced a distributional shift that harmed performance on realistic low-resolution images. The validation PSNR decrease from 24.72 dB in Stage 1 to 24.28 dB in Stage 2 further indicates that the model began adapting to synthetic degradation patterns at the expense of real-world degradation handling.
A fundamental limitation of the EDSR approach in this project was the disconnect between the SR training objective and the downstream ReID task. EDSR was trained to minimize pixel-wise L1 loss against ground-truth HR images, but this objective does not directly optimize for preserving or enhancing features that are discriminative for person re-identification. The SR module had no mechanism to learn which image details are most relevant for ReID embeddings, which may explain why the Sequential model configuration showed substantially degraded ReID performance despite the SR module providing measurable PSNR improvements. This observation aligns with the findings in Section \ref{sec: related work} regarding the risk of applying off-the-shelf SR models directly to person ReID problems.
The perception-distortion trade-off also imposes inherent constraints on what EDSR can achieve. By prioritizing low distortion, the model produces smoother outputs that minimize pixel-wise error but may lack the high-frequency details that perceptual quality-focused models introduce. While this is intentional to avoid hallucinated artifacts, it means the SR outputs may not appear significantly sharper to visual inspection despite achieving higher PSNR values. For the ReID application, this conservative approach was appropriate, as introducing false details could mislead the ReID module, but it also means the SR module's contribution to image quality enhancement is fundamentally bounded.
...
The \acs{SR} module could be improved through several approaches. Implementing an auxiliary loss during End-to-end training would allow the SR module to optimize for both reconstruction quality and \acs{ReID} performance at the same time, rather then only learning from only learning from the \acs{ReID} loss. The \acs{SR} network could be designed to specifically  enhance discriminative features such as clothing patterns, logos and textures that are particularly relevant for person identification, rather then generic image enhancement.  Furthermore, developing a multi-scale SR approach that can handle varying input resolutions would improve robustness across different camera qualities and different distances which are essential for real-world surveillance scenarios. 
