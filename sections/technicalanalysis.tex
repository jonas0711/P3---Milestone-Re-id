...
Early studies showed that deep-learning could replace hand-crafting features for \ac{ReID} tasks, by automatically learning features directly from image data. Later research, specifically the Omniscale paper, introduced a lightweight architecture that could capture features across different scales, combining global and local features. The more recent study from 2024 emphasized how image quality plays a critical role in \ac{ReID} accuracy. All three papers use either Rank-1, mAP or both to measure accuracy. This leaves the important takeaways as:
...
\begin{itemize}
    ...
    \item Rank-1 and mAP is used for accuracy. 
\end{itemize}

\subsection{Out-of-Distribution Test of OSNet}\label{outofdistributiontest}

An out-of-distribution (OOD) test is defined as the models ability to perform on data that is significantly different from the data it was trained on \cite{OOD}. The difference is determined as significant by measures such as image vs. cartoon, all outdoor vs. indoor, and farm animals vs. wild animals. This project determines the OOD test as data that is abnormal to the model, in the form of augmentated images and unseen data from a different environment.
\\\\
As discussed in Section \ref{subsec:reid_reworks}, OSNet appears to be the most promising network, as it is omni-scale and thus scale-invariant. However, the authors of the paper conducted an experiment of testing the model on images reduced with a scale factor 0.25, corresponding to a 64x32 image \cite{OSNet}. Its performance showed a drop in Rank-1 accuracy from 94.8\% to 86.9\% , while the mAP decreases from 84.9\% to 67.3\% on the Market-1501 dataset. These results indicate that OSNet is sensitive to changes in the scale of the image, despite the omni-scale architecture.
\\\\
OSNet has also been evaluated on two difficult \ac{ReID} datasets, VIPeR and GRID, which have a small amount of images, containing only two instances/examples of each individual. Although the test achieved state-of-the-art results, the Rank-1 accuracies remained relatively low at 68.0\% and 38.2\% respectively. This further suggests that OSNet struggles to generalize to other and more difficult datasets.
\\\\
For these reasons, OSNet will be evaluated further to confirm or deny the assumption that existing \ac{ReID} algorithms have difficulties identifying people when the image resolution is poor due to scale or compression in surveillance. The OOD test is conducted using a small subset of images from the dataset Market-1501 under five different conditions, as well as a subset from the dataset iLIDS-VID which is unknown to OSNet. These subsets are presented in Table \ref{tab:ood_conditions}.
...
\noindent Each pair is of the same individual captured by a different camera, structured such that one images serves as the query  and the other as the gallery sample. An example pair of gallery and query from the selected images is shown in Figure \ref{fig:test_images_outofdistribution}. All image names are listed in Appendix \ref{app:out-of-dis}, sorted into query and gallery.

\begin{figure} [H]
    \centering
    \includegraphics[width=1\linewidth]{out_of_distribution_testimages.png}
    \caption{Image examples from the test. The first row consists of gallery images, while the second row consists query images.}
    \label{fig:test_images_outofdistribution}
\end{figure}

\noindent The test is conducted using the \texttt{Torchreid} library, which provides integrated data managers, model loading, and evaluation tools. This enables quick testing of existing models, although it limits insight in the underlying process. However, this is found sufficient for this initial evaluation. \\
Link: \texttt{https://kaiyangzhou.github.io/deep-person-reid/index.html}
\\\\
The selected model is \texttt{osnet\_x1\_0}, the largest and highest performing base OSNet variant, consisting of 22 million parameters and 0.98G FLOPs. It is pretrained on MSMT17, DukeMTMC-reID and CUHK03, and evaluated on Market-1501. Among the options of pretrained OSNet models, which are trained and evaluated across different dataset, this version has a good baseline performance of 72.5 Rank-1 and 44.2 mAP. Training has used cosine distance, softmax classifier and max epoch of training sat at 50.

\subsubsection{Unmodified Images Test}

The model achieves a Rank-1 accuracy of 52.0\% and a mAP of 63.4\% on the unmodified images. These results are notably lower than the performance reported in the original OSNet paper. This discrepancy may be attributed to the small sample size and differences in data selection and evaluation setup.

\subsubsection{Different Scale Test}

As the resolution is reduced, performance declines, most visible in the drop in mAP and Rank-1 accuracy, with the results presented in Table \ref{tab:scaletest_result}. This confirms that OSNet is sensible to changes in scale, confirming the assumption, that \ac{ReID} models struggle when presented with low resolution surveillance data.
...
\rowcolor[HTML]{D8E9F7}
{} & {Scale 1 (128x64)} & {Scale 0.75 (96x48)} & {Scale 0.5 (64x32)} &
{Scale 0.25 (32x16)}\\ \hline
\textbf{mAP}    & 63.4\%  & 57.5\%  & 46.4\% & 49.8\% \\ \hline
\textbf{Rank-1} & 52.0\%  & 48.0\%  & 36.0\% & 40.0\% \\ \hline
\textbf{Rank-5} & 76.0\%  & 64.0\%  & 72.0\% & 68.0\% \\ \hline
\textbf{Rank-10}& 84.0\%  & 72.0\%  & 84.0\% & 72.0\% \\ \hline
\textbf{Rank-20}& 100.0\% & 96.0\%  & 96.0\% & 96.0\% \\ \hline
\end{tabular}
...
\\\\
Applying JPEG compression to both the query and gallery affects the performance of OSNet, as can be seen in Table \ref{tab:compressiontest_result}. The mild compression 75 does not change the performance, whereas increased compression, 50 and 25, leads to a clear decline in accuracy. These results show that image compression negatively impacts model performance, particularly at lower quality levels. It is worth noting that the Market-1501 images were already subject to JPEG compression. Therefore the compression performed for this test is in addition to prior compression, and does not necessarily simulate real-world conditions.
...
Heavy 25\\ \hline
\textbf{mAP}    & 63.4\%& 63.4\%& 59.2\%& 53.9\%\\ \hline
\textbf{Rank-1} & 52.0\%& 52.0\%& 44.0\%& 44.0\%\\ \hline
\textbf{Rank-5} & 76.0\%& 76.0\%& 80.0\%& 68.0\%             \\ \hline
\textbf{Rank-10}& 84.0\%& 84.0\%& 92.0\%& 88.0\%\\ \hline
...
\subsubsection{Abnormal and Unseen Data Test}

Both the abnormal image subset and the subset of new images result in substantially lower mAP and Rank-1 to Rank-20 scores, compared to the unmodified images, as presented in Table \ref{tab:othertest_result}. This outcome is expected, as the abnormal samples only contain cropped or partially visible individuals, making matching significantly more difficult. Additionally, the iLIDS-VID dataset exclusively contains indoor scenes, whereas datasets used for training and evaluation consists primarily of outdoor images. This shift in domain is likely contributing to the reduced performance on the iLIDS-VID subset. 
...
\begin{tabular}{|>{\centering\arraybackslash}
    ...
    |}
\hline
\multicolumn{4}{|c|}{\cellcolor[HTML]{D8E9F7}\textbf{Test on Abnormal and Unseen Data}}\\\hline
\rowcolor[HTML]{D8E9F7}
{} & Unmodifed (Market-1501)& Abnormal (Market-1501)& New dataset (iLIDS-VID)\\ \hline
\textbf{mAP}    & 63.4\%& 45.0\%& 35.1\%\\ \hline
\textbf{Rank-1} & 52.0\%& 32.0\%& 16.0\%\\ \hline
\textbf{Rank-5} & 76.0\%& 60.0\%& 60.0\%\\ \hline
\textbf{Rank-10}& 84.0\%& 72.0\%& 76.0\%\\ \hline
\textbf{Rank-20}& 100.0\%& 84.0\%& 96.0\%\\ \hline
\end{tabular}
\caption{Abnormal and unseen data test of OSNet on 25 pairs from Market-1501 dataset, and iLIDS-VID dataset.}
\label{tab:othertest_result}
\end{table}

\subsubsection{Summary}

Overall, these results show a clear reduction in performance when OSNet is evaluated on data that is different than what it was primarily trained on. This highlights challenges with generalization, particularly regarding new data, person visibility/occlusion, scale, and resolution. The last two are areas that may improve with integration of \ac{SR}.
...
The SRCNN paper demonstrated that deep learning could successfully learn mapping directly between low- and high-resolution images. Later research, specifically the SAN paper, improved on this by introducing attention-based techniques that model feature correlations and global image context. The two methods both evaluate their results through PSNR and SSIM metrics. This leaves the important takeaways as:
...
\section{Overgang, vinkel}
MANGLER  
\\\\
Vinkel, bare tanker: Deployed proof-of-concept person re-id system, combined with super res to enhance performance, because of bad image quality. This is because of compression (?) or else surveillance would be very expensive. A solution could therefore be super res. Focus is also generalizability. The models proposed by the papers we have read are not very generalizable, - check our test. It would be beneficial to create a model that can actually generalize - this gives it value in deployment. 
\\\\
Overgang til næste kapitel. Hvad er vores vinkel RIS OP. Generalisere til real-world deployment? 